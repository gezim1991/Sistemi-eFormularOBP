{% load i18n %} {% load rest_framework %} {% trans "No items to select." as no_items %}
...
<div class="form-group">
  ...
  <div class="col-sm-10">
    <select multiple class="form-control" name="{{ field.name }}">
      ...
      <optgroup label="{{ select.label }}">{% elif select.end_option_group %}</optgroup>
      {% else %}
      <option
        value="{{ select.value }}"
        {%
        if
        select.value|as_string
        in
        field.value|as_list_of_strings
        %}selected{%
        endif
        %}
        {%
